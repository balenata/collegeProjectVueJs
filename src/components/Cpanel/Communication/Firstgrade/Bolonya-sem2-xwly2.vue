              ...
              <v-row>
                ...
                <v-col cols="12" md="6">
                  ...
                  <v-btn @click="Passed()" color="grey"
                    >Stage 2: Semester 1: Xwly 1</v-btn
                  >
                </v-col>
              </v-row>
            </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    UpdateUser(data) {
      ...
      fs.append('current_stage', 1)
      fs.append('current_semester', 2)
      fs.append('current_xwl', 2)
      axios
        .put(`communication-student/${this.updateId.id}/?g=1&s=2&x=2`, fs, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(() => {
          this.dialog2 = false
    ...
    DeleteUser() {
      const accesToken = localStorage.getItem('access_token')
      axios
        .delete(`communication-student/${this.deleteId}/?g=1&s=2&x=2`, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(res => {
          this.dialog1 = false
