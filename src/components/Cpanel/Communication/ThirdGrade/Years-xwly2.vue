          ...
          <v-card style="overflow-x: hidden;">
            ...
            <v-card-actions>
              <v-row>
                <v-col cols="12" md="6">
                  Failed:
                  <v-btn @click="Failed()" color="grey">Xwly 1</v-btn>
                </v-col>
                <v-col cols="12" md="6">
                  Move to:
                  <v-btn @click="Passed()" color="grey">Stage 4</v-btn>
                </v-col>
              </v-row>
            </v-card-actions>
          </v-card>
...
export default {
  ...
  methods: {
    ...
    UpdateUser(data) {
      ...
      axios
        .put(`communication-student/${this.updateId.id}/?g=3&s=0&x=2`, fs, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(() => {
          this.dialog2 = false
    ...
    DeleteUser() {
      const accesToken = localStorage.getItem('access_token')
      axios
        .delete(`communication-student/${this.deleteId}/?g=3&s=0&x=2`, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(res => {
          this.dialog1 = false
