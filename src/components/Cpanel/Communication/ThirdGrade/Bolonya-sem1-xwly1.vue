...
export default {
  ...
  methods: {
    ...
    UpdateUser(data) {
      ...
      fs.append('current_stage', 3)
      fs.append('current_semester', 1)
      fs.append('current_xwl', 1)
      axios
        .put(`communication-student/${this.updateId.id}/?g=3&s=1&x=1`, fs, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(() => {
          this.dialog2 = false
    ...
    DeleteUser() {
      const accesToken = localStorage.getItem('access_token')
      axios
        .delete(`communication-student/${this.deleteId}/?g=3&s=1&x=1`, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(res => {
          this.dialog1 = false
