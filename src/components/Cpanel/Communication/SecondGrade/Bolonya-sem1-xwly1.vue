<template>
  <div>
    ...
    <v-card>
        ...
        >
          <v-card>
            ...
            <v-card-actions>
              <v-row>
                <v-col
                  v-for="(bolonya, i) in infoId.bolonya_system"
                  :key="i"
                  cols="12"
                  md="12"
                >
      ...
      <!-- LEVELUP USER  -->
      <v-row justify="center">
        <v-dialog v-model="levelUpDialog" max-width="550" max-height="350">
          <v-card style="overflow-x: hidden;">
            ...
            <v-card-actions>
              <v-row>
                <v-col cols="12" md="6">
                  Move to:
                  <v-btn @click="Failed()" color="grey"
                    >Semester 1: Xwly 2</v-btn
                  >
                </v-col>
                <v-col cols="12" md="6">
                  Move to:
                  <v-btn @click="Passed()" color="grey"
                    >Semester 2: Xwly 1</v-btn
                  >
                </v-col>
              </v-row>
            </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    UpdateUser(data) {
      ...
      fs.append('phone_number', Number(data.phone_number))
      fs.append('department', 'communication')
      fs.append('current_stage', 2)
      fs.append('current_semester', 1)
      fs.append('current_xwl', 1)
      axios
        .put(`communication-student/${this.updateId.id}/?g=2&s=1&x=1`, fs, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(() => {
          this.dialog2 = false
    ...
    DeleteUser() {
      const accesToken = localStorage.getItem('access_token')
      axios
        .delete(`communication-student/${this.deleteId}/?g=2&s=1&x=1`, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(res => {
          this.dialog1 = false
    ...
    UpdateInfo(data) {
      ...
      fb.append('semester', Number(data.semester))
      ...
      axios
        .put(`bolonya-communication/${data.id}/`, fb, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(() => {
    ...
    DeleteInfo(data) {
      const accesToken = localStorage.getItem('access_token')
      axios
        .delete(`bolonya-communication/${data.id}/`, {
          headers: { Authorization: 'Bearer ' + accesToken }
        })
        .then(() => {
